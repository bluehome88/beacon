...
class Securimage {
   ...
   * @var string
   */
  var $multi_text_color = "#2167CE";//#f65c47,#8d32fd";
...
//    if ($this->arc_linethrough == true) $this->arcLines();
  ...
  function readCodeFromFile()
  {
    ...
    return substr($data, $start, $end - $start); // return substring in 128 bytes
  }
  ...
  function validate()
  {
    if ( isset($_SESSION['securimage_code_value']) && !empty($_SESSION['securimage_code_value']) ) {
      if ( $_SESSION['securimage_code_value'] == trim($this->code_entered) ) {
