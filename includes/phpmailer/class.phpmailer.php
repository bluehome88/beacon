...
class mosPHPMailer
{
	...
	*/
	...
	function Send() {
		...
		switch($this->Mailer)
		{
			...
			case "smtp":
				...
				break;
			default:
			$this->SetError($this->Mailer . 'mailer is not supported.');
				$result = false;
				break;
		}
	...
	function SendmailSend($header, $body) {
		...
		{
			$this->SetError("Could not execute: " . $this->Sendmail);
			return false;
		}
	...
	function SmtpSend($header, $body) {
		...
		{
			$error = "The following From address failed: " . $smtp_from;
			$this->SetError($error);
			$this->smtp->Reset();
			return false;
		}
	...
	function SmtpConnect() {
		...
		if(!$connection)
			$this->SetError("SMTP Error: Could not connect to SMTP host.");
	...
	function AddAttachment($path, $name = "", $encoding = "base64",
		...
		if(!@is_file($path))
		{
			$this->SetError("Could not access file: " . $path);
			return false;
		}
	...
	function EncodeFile ($path, $encoding = "base64") {
		if(!@$fd = fopen($path, "rb"))
		{
			$this->SetError("File Error: Could not open file: " . $path);
			return "";
		}
